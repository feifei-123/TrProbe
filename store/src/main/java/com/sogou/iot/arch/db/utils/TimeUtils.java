...
//        long timeStamp, timeCur;
//        long dayTime = 1000 * 60 * 60 * 24;
//        timeStamp = getHappentime(dayStr(stamp));
//        timeCur = getHappentime(dayStr(System.currentTimeMillis()));
//        Calendar cdStamp = Calendar.getInstance();
//        cdStamp.setHappentime(new Date(stamp));
...
//    }
    ...
    public static int getCurrentWeek(){
        Calendar cdStamp = Calendar.getInstance();
        cdStamp.setTime(new Date());
        int daytime = cdStamp.get(Calendar.DAY_OF_WEEK);
        int result = 1;
        switch (daytime) {
            case Calendar.MONDAY:
                result = 1;
